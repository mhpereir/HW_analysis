...
Both terms are Stage-2 sums over the inclusive 48-hour pre-peak window
`(-48, 0)`. The Stage-2 builder stores the result as `I_dyn_pre`. Positive
...
The numerical results and figures below are a retained historical snapshot
from the former 96-hour `I_dyn_pre` product. They were regenerated by PBS job
`2589929.venus` at source commit
