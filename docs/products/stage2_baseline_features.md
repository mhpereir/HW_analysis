...
Floating-point sums and means follow the event-feature numerical contract:
they must be equivalent to direct reductions of each selected local window and
must not lose precision because of the preceding Stage-1 record length or
magnitude.
...
The shared reducer must also be tested with long, high-magnitude histories,
near-cancelling local windows, and missing values. Products generated by a
reducer that fails those direct-reduction comparisons must be regenerated;
their dimensions, variables, window definitions, and baseline populations
remain compatible.
