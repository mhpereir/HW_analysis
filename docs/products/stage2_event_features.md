...
Floating-point reductions must be numerically equivalent to reducing the
selected local-window samples directly. Their accuracy must not depend on the
magnitude or length of the Stage-1 record preceding a window. In particular,
implementations must avoid subtracting full-record floating-point cumulative
totals when that loses precision relative to a direct local-window reduction.
...
Shared reduction tests must include long, high-magnitude histories and
near-cancelling local windows, and must compare sums and means with direct
reductions of the selected samples. Products generated by a reducer that does
not satisfy this requirement must be regenerated; their dimensions, variable
names, window definitions, and selector populations remain compatible.
